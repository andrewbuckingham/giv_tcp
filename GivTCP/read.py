...
import os
# Import utility functions (Phase 1 refactoring: testing infrastructure)
from utils import dicttoList, iterate_dict, dataSmoother2

# Phase 2 refactoring: Repository pattern for thread-safe cache operations
from repositories import PickleCacheRepository
...
# Phase 2: Initialize cache repository
# Feature flag: set environment variable USE_NEW_CACHE=true to enable
USE_NEW_CACHE = os.getenv('USE_NEW_CACHE', 'false').lower() == 'true'
if USE_NEW_CACHE:
    cache_repo = PickleCacheRepository(GiV_Settings.cache_location)
    logger.info("Using new PickleCacheRepository for cache operations")
else:
    cache_repo = None
    logger.info("Using legacy pickle operations for cache")

# Phase 5: Initialize lock manager
# Feature flag: set environment variable USE_NEW_LOCKS=true to enable
USE_NEW_LOCKS = os.getenv('USE_NEW_LOCKS', 'false').lower() == 'true'
if USE_NEW_LOCKS:
    lock_manager = ThreadLockManager()
    logger.info("Using new ThreadLockManager for synchronization")
else:
    lock_manager = None
...
def getData(fullrefresh):  # Read from Invertor put in cache
    ...
    # Phase 5: Use new lock manager or legacy file-based locking
    # Connect to Invertor and load data
    try:
        if USE_NEW_LOCKS:
            # New: Use lock manager with timeout (eliminates TOCTOU vulnerabilities)
            try:
                ...
                result['result'] = "Error: Timeout waiting for inverter read lock"
                return json.dumps(result)
        else:
            # Legacy: File-based locking (has TOCTOU vulnerability)
        ...
        # Common code for both locking mechanisms
        multi_output['Last_Updated_Time'] = datetime.datetime.utcnow().replace(tzinfo=datetime.timezone.utc).isoformat()

        # Get lastupdate from cache (Phase 2: using repository if enabled)
        if USE_NEW_CACHE:
            previousUpdate = cache_repo.get('lastUpdate_' + str(GiV_Settings.givtcp_instance))
            if previousUpdate:
            ...
            # Save new time to cache
            cache_repo.set('lastUpdate_' + str(GiV_Settings.givtcp_instance), multi_output['Last_Updated_Time'])
        else:
            # Legacy pickle operations
...
# Energy Today Figures
        ...
        # Reset cache at midnight (Phase 2: using repository if enabled)
        if checksum == 0 and GEInv.system_time.hour == 0 and GEInv.system_time.minute == 0:
            ...
            if USE_NEW_CACHE:
                if cache_repo.exists('regCache_' + str(GiV_Settings.givtcp_instance)):
                    cache_repo.delete('regCache_' + str(GiV_Settings.givtcp_instance))
            else:
                # Legacy pickle operations
...
######## Grab output history to allow data smoothing ########
        ...
        # Load regCache (Phase 2: using repository if enabled)
        if USE_NEW_CACHE:
            regCacheStack = cache_repo.get('regCache_' + str(GiV_Settings.givtcp_instance))
            if regCacheStack:
...
######### Section where post processing of multi_ouput functions are called ###########
        ...
        # Save new data to cache (Phase 2: using repository if enabled)
        if USE_NEW_CACHE:
            cache_repo.set('regCache_' + str(GiV_Settings.givtcp_instance), regCacheStack)
        else:
            # Legacy pickle operations
...
def pubFromPickle():  # Publish last cached Invertor Data
    multi_output = {}
    result = "Success"
    # Phase 2: Check for cache using repository if enabled
    if USE_NEW_CACHE:
        regCacheStack = cache_repo.get('regCache_' + str(GiV_Settings.givtcp_instance))
        if not regCacheStack:  # if there is no cache, create it
            result = getData(True)
            regCacheStack = cache_repo.get('regCache_' + str(GiV_Settings.givtcp_instance))
        if regCacheStack and "Success" in result:
...
def publishOutput(array, SN):
    tempoutput = {}
    tempoutput = iterate_dict(array, logger)
...
# iterate_dict function moved to utils.py (Phase 1 refactoring)
...
def ratecalcs(multi_output, multi_output_old):
    ...
    # check if cache data exists (Phase 2: using repository if enabled):
    if USE_NEW_CACHE:
        rate_data = cache_repo.get('rateData_' + str(GiV_Settings.givtcp_instance))
        if not rate_data:
            rate_data = {}
    else:
        # Legacy pickle operations
    ...
    # dump current data to cache (Phase 2: using repository if enabled)
    if USE_NEW_CACHE:
        cache_repo.set('rateData_' + str(GiV_Settings.givtcp_instance), rate_data)
    else:
        # Legacy pickle operations
...
# dicttoList function moved to utils.py (Phase 1 refactoring)
...
def loop_dict(array, regCacheStack, lastUpdate):
    ...
    for p_load in array:
        ...
        if isinstance(output, dict):
            ...
            if p_load in regCacheStack:
                safeoutput[p_load] = dataSmoother2([p_load, output], [p_load, regCacheStack[p_load]], lastUpdate, givLUT, GivLUT.timezone, GiV_Settings.data_smoother)
            else:
...
# dataSmoother2 function moved to utils.py (Phase 1 refactoring)


def calcBatteryValue(multi_output):
    # get current data from cache (Phase 2: using repository if enabled)
    batterystats = {}
    if USE_NEW_CACHE:
        batterystats = cache_repo.get('battery_' + str(GiV_Settings.givtcp_instance))
        if not batterystats:
            # First time running
            logger.critical("First time running so saving AC Charge status")
            batterystats = {
                'AC Charge last': float(multi_output['Energy']['Total']['AC_Charge_Energy_Total_kWh']),
                'Battery_Value': 0,
                'Battery_ppkwh': 0,
                'Battery_kWh_old': multi_output['Power']['Power']['SOC_kWh']
            }
    else:
        # Legacy pickle operations
    ...
    if GiV_Settings.first_run or datetime.datetime.now(GivLUT.timezone).minute == 59 or datetime.datetime.now(GivLUT.timezone).minute == 29:
        # Check for touchfile and battery stats (Phase 2: using repository if enabled)
        has_battery_stats = USE_NEW_CACHE and cache_repo.exists('battery_' + str(GiV_Settings.givtcp_instance)) or (not USE_NEW_CACHE and exists(GivLUT.batterypkl))
        if not exists(GivLUT.ppkwhtouch) and has_battery_stats:      # only run this if there is no touchfile but there is a battery stat
    ...
    # write data to cache (Phase 2: using repository if enabled)
    if USE_NEW_CACHE:
        cache_repo.set('battery_' + str(GiV_Settings.givtcp_instance), batterystats)
    else:
        # Legacy pickle operations
