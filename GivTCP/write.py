...
if USE_NEW_CACHE:
    ...
    logger.info("Write module: Using new PickleCacheRepository for cache operations")
else:
    cache_repo = None
    logger.info("Write module: Using legacy pickle operations for cache")


def get_cached_data():
    """Helper function to get cached data (Phase 2: using repository if enabled)."""
    if USE_NEW_CACHE:
        regCacheStack = cache_repo.get('regCache_' + str(GiV_Settings.givtcp_instance))
        if regCacheStack:
            return regCacheStack[4]
        return None
    else:
...
def forceExport(exportTime):
    ...
    try:
        ...
        # Get cached data (Phase 2: using repository if enabled)
        multi_output = get_cached_data()
        if multi_output:
            revert["dischargeRate"]=multi_output["Control"]["Battery_Discharge_Rate"]
            revert["start_time"]=multi_output["Timeslots"]["Discharge_start_time_slot_1"][:5]
            revert["end_time"]=multi_output["Timeslots"]["Discharge_end_time_slot_1"][:5]
            revert["dischargeToPercent"]=multi_output["Control"]["Battery_Power_Reserve"]
            revert["mode"]=multi_output["Control"]["Mode"]

        #set slot2 to calc times and keep slot 1 as is
        slot1=(datetime.strptime(multi_output["Timeslots"]["Discharge_start_time_slot_1"][:5],"%H:%M"),datetime.strptime(multi_output["Timeslots"]["Discharge_end_time_slot_1"][:5],"%H:%M")) 
...
def forceCharge(chargeTime):
    ...
    try:
        ...
        multi_output = get_cached_data()
        if multi_output:
            revert["start_time"]=multi_output["Timeslots"]["Charge_start_time_slot_1"][:5]
            revert["end_time"]=multi_output["Timeslots"]["Charge_end_time_slot_1"][:5]
            revert["chargeRate"]=multi_output["Control"]["Battery_Charge_Rate"]
            revert["targetSOC"]=multi_output["Control"]["Target_SOC"]
            revert["chargeScheduleEnable"]=multi_output["Control"]["Enable_Charge_Schedule"]
...
def tempPauseDischarge(pauseTime):
    ...
    try:
        ...
        # Get cached data (Phase 2: using repository if enabled)
        multi_output = get_cached_data()
        revertRate = multi_output["Control"]["Battery_Discharge_Rate"] if multi_output else 0
...
def tempPauseCharge(pauseTime):
    ...
    try:
        ...
        # Get cached data (Phase 2: using repository if enabled)
        multi_output = get_cached_data()
        revertRate = multi_output["Control"]["Battery_Charge_Rate"] if multi_output else 0
